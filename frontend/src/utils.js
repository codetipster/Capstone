...
export function calculateAverageRates(data) {
  ...
  for (let index = 0; index < data.report.length; index += 1) {
    let dailyTotalViewRate = 0;
    let dailyTotalCustomViewRate = 0;

    data.report[index].adslots.forEach((adslot) => {
      dailyTotalViewRate += adslot.average_view_rate;
      dailyTotalCustomViewRate += adslot.average_custom_view_rate;
    });

    const dailyAverageViewRate = dailyTotalViewRate / data.report[index].adslots.length;
    const dailyAverageCustomViewRate = dailyTotalCustomViewRate / data.report[index].adslots.length;
