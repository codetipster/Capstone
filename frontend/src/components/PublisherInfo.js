...
const PublisherInfo = () => {
  ...
  const averageViewRateData = getChartDataForRechartsOnAverageViewRate();

  const chartData = {
    labels: averageViewRateData.map(item => item.date),
    datasets: ['sky', 'mrec', 'superbanner', 'banner'].map((type, index) => ({
      label: type,
      data: averageViewRateData.map(item => item[type]),
